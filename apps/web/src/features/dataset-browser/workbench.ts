...
const DOMAIN_SUMMARIES: Record<DatasetDomain, string> = {
  market: "适合看价格、成交量和基础行情结构，是最常见的研究入口。",
  derivatives: "适合看资金费率、持仓量和拥挤度，偏执行与情绪视角。",
  on_chain: "适合看链上活跃度、地址行为和资金迁移路径。",
  macro: "适合看宏观背景、风险偏好和跨市场因子。",
  sentiment_events: "适合看新闻、事件和情绪冲击对市场的影响。",
};

const TYPE_SUMMARIES: Record<DatasetType, string> = {
  display_slice: "更适合浏览与看图，帮助快速理解这份数据本体。",
  training_panel: "更适合训练模型，特征、标签和切分规则已经被整理成统一面板。",
  feature_snapshot: "更适合作为上游特征资产复用，供研究或训练链路调用。",
  fusion_training_panel: "将多个数据域按显式对齐规则物化成统一训练面板，适合跨域训练与可追溯复盘。",
};
...
const PRETTY_LABELS: Record<string, string> = {
  ...
  data_source: "数据来源",
  dataset_type: "数据类型",
  drop_unaligned_rows: "是否丢弃未对齐样本",
  end_time: "结束时间",
  entity_count: "实体数量",
  entity_scope: "实体范围",
  exchange: "交易所",
  feature_count: "特征维度",
  feature_schema_hash: "特征 Schema Hash",
  feature_set_id: "特征集合",
  frequency: "频率",
  join_key: "对齐主键",
  label_count: "标签数",
  ...
  min_entity_coverage_ratio: "最小实体覆盖率",
  missing_feature_policy: "缺失特征策略",
  quality_status: "质量状态",
  readiness_status: "训练就绪度",
  request_name: "请求名称",
  request_origin: "请求来源",
  sample_count: "样本条数",
  sample_policy: "样本策略",
  ...
  symbol_type: "资产类型",
  symbols: "资产列表",
  symbols_preview: "资产预览",
  temporal_safety_status: "时间安全",
  time_window: "时间窗口",
  usable_sample_count: "可用样本条数",
};
