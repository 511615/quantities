...
const JOB_TYPE_LABELS: Record<string, () => string> = {
  ...
  dataset_multimodal_train: () => translateText("多模态训练闭环"),
...
const STAGE_NAME_LABELS: Record<string, () => string> = {
  ...
  prepare_dataset: () => translateText("检查数据集"),
  prepare: () => I18N.stage.prepare,
  train: () => I18N.stage.train,
  train_market: () => translateText("训练市场模态"),
  train_macro: () => translateText("训练宏观模态"),
  train_on_chain: () => translateText("训练链上模态"),
  train_derivatives: () => translateText("训练衍生品模态"),
  train_nlp: () => translateText("训练 NLP 模态"),
  inspect: () => translateText("校验融合输入"),
  compose: () => translateText("生成融合模型"),
  predict: () => I18N.stage.predict,
  backtest: () => I18N.stage.backtest,
  readiness: () => translateText("就绪校验"),
  review: () => translateText("结果复核"),
};
...
const ARTIFACT_LABELS: Record<string, () => string> = {
  ...
  backtest_leakage: () => translateText("回测泄漏审计"),
  pnl: () => translateText("收益拆解"),
  backtest_pnl: () => translateText("回测收益拆解"),
