...
import { DatasetRenameDialog } from "../features/dataset-browser/DatasetRenameDialog";
...
export function DatasetsBrowserPage() {
  ...
  const [renameTarget, setRenameTarget] = useState<{ datasetId: string; label: string } | null>(null);
  ...
  return (
    <div className="page-stack">
                  ...
                  <tr key={item.datasetId}>
                    <td>
                      <div className="table-title-cell">
                        ...
                        <span>{`申请时间：${formatDate(item.requestedAt)}`}</span>
                    ...
                    <td>
                      <div className="table-actions">
                        ...
                        <button
                          ...
                          onClick={() => setRenameTarget({ datasetId: item.datasetId, label: item.title })}
                          type="button"
                        >
                          改名
                        </button>
      ...
      <DatasetRenameDialog
        currentLabel={renameTarget?.label ?? "数据集"}
        datasetId={renameTarget?.datasetId ?? null}
        onClose={() => setRenameTarget(null)}
        onRenamed={(result) => {
          setRenameTarget(null);
          setRenameNotice(`已将 ${result.dataset_id} 更新为 ${result.display_name}。`);
        }}
      ...
      <DatasetDeleteDialog
        ...
        onDeleted={(result) => {
          ...
          setDeleteNotice(`数据集 ${result.dataset_id} 已从注册表中移除。`);
        }}
