    ...
    <div className="page-stack">
      <section className="page-header-shell">
        <div className="page-header-main">
          ...
          <h1>训练面板总览</h1>
          <p>这里只保留真正适合进入训练比较的面板，帮助快速判断哪些数据值得继续推进。</p>
        </div>
        ...
        <>
          ...
          <section className="panel">
            <PanelHeader
              eyebrow="训练比较"
              title="训练面板目录"
              description="先比较样本量、特征维度、标签窗口和切分方式，再决定用哪份数据发起训练。"
            />
            <div className="dataset-browser-table-shell">
              <table className="data-table">
                <thead>
                  <tr>
                    <th>名称</th>
                    <th><TermLabel hintKey="dataset_type" label="数据集类型" /></th>
                    <th>研究范围</th>
                    <th>样本量</th>
                    <th><TermLabel hintKey="feature_dimensions" label="特征维度" /></th>
                    <th>标签列</th>
                    <th><TermLabel hintKey="label_horizon" label="标签窗口" /></th>
                    <th><TermLabel hintKey="split_strategy" label="切分方式" /></th>
                    <th><TermLabel hintKey="freshness" label="新鲜度" /></th>
                    <th>质量</th>
                    <th><TermLabel hintKey="training_panel" label="训练就绪度" /></th>
                    <th>操作</th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    <tr key={item.datasetId}>
                      <td>
                        <div className="table-title-cell">
                          ...
                          <span>版本：{item.snapshotVersion}</span>
                        </div>
                      </td>
                      <td>
          ...
          {blockedItems.length > 0 ? (
            <section className="panel">
              <PanelHeader
                eyebrow="暂不可训练"
                title="当前不建议直接推进的面板"
                description="这些数据先不进入主比较表，避免把“能浏览”误认成“能训练”。"
              />
