...
export function HomePage() {
  ...
  if (!overviewQuery.data) {
    return <EmptyState body={translateText("当前没有可展示的工作台概览数据。")} title={I18N.state.empty} />;
  }
  ...
  const riskItems = [
    ...overview.recentJobs.filter((job) => job.status === "failed").map((job) => ({
      ...
      body: translateText("任务状态为失败，建议先回到任务中心查看阶段和错误说明。"),
    })),
    ...overview.recentBacktests
      ...
      .map((item) => ({
        ...
        body: `${translateText("告警")} ${item.warning_count} ${translateText("条")} / ${translateText("状态")} ${formatStatusLabel(item.status)}`,
      })),
  ].slice(0, 4);
    ...
    <div className="page-stack home-page">
      <section className="page-header-shell">
        <div className="page-header-main">
          ...
          <p>{translateText("把最近活动、关键入口和风险提示压缩到同一屏里，让你先看清现在发生了什么，再决定下一步。")}</p>
        </div>
      ...
      <div className="workspace-grid workspace-grid-balanced">
        <section className="workspace-primary page-stack">
          <section className="panel">
            <PanelHeader
              eyebrow={translateText("当前流转")}
              title={translateText("最近活动")}
              description={translateText("用同一视角跟踪训练、回测和任务状态，不再把它们分散在不同页面才能看全。")}
            />
              ...
              {overview.recentBacktests.slice(0, 2).map((item) => (
                <div className="activity-row" key={item.backtest_id}>
                  ...
                  <div className="activity-copy">
                    ...
                    <span>
                      <GlossaryHint hintKey="max_drawdown" /> {formatNumber(item.max_drawdown)} / {translateText("告警")} {item.warning_count} {translateText("条")}
                    </span>
                  </div>
                  <StatusPill status={item.status} />
                </div>
          ...
          <section className="panel">
            <PanelHeader
              eyebrow={translateText("关键入口")}
              title={translateText("下一步最常用的操作")}
              description={translateText("把模型、数据集、回测和基准入口做成清晰的工作流跳板，而不是四块同质化卡片。")}
            />
            <div className="quick-link-grid">
              ...
              <Link className="nav-panel-link" to="/backtests">
                ...
                <span>{`${translateText("最近")} ${overview.recentBacktests.length} ${translateText("条回测结果")}`}</span>
              </Link>
              <Link className="nav-panel-link" to="/benchmarks">
                <strong>{I18N.nav.benchmarks}</strong>
                <span>{overview.recentBenchmarks[0]?.benchmark_name ?? translateText("暂无基准")}</span>
              </Link>
            </div>
          </section>
        </section>
        ...
        <aside className="workspace-sidebar page-stack">
          <section className="panel">
            <PanelHeader
              eyebrow={translateText("待处理项")}
              title={translateText("风险提示")}
              description={translateText("失败任务、高告警回测和可疑结果优先出现在右侧，帮助你先处理风险。")}
            />
            {riskItems.length > 0 ? (
              ...
              <EmptyState body={translateText("当前没有新的高风险项，可继续浏览最近训练实例或基准对比。")} title={translateText("风险可控")} />
            )}
          </section>

          <section className="panel">
            <PanelHeader
              eyebrow={translateText("基准快照")}
              title={translateText("当前 benchmark 状态")}
              description={translateText("首页只保留适合扫读的 benchmark 摘要，详细分析统一收敛到 benchmark 页。")}
            />
              ...
              <div className="stack-list">
                {overview.recentBenchmarks.map((item) => (
                  <div className="stack-item align-start" key={item.benchmark_name}>
                    ...
                    <span>{`${translateText("当前领先")} ${item.top_model_name ?? "--"} / ${translateText("得分")} ${formatNumber(item.top_model_score)}`}</span>
                  </div>
                ))}
              </div>
            ) : (
              <EmptyState body={translateText("暂无基准摘要。")} title={I18N.state.empty} />
            )}
          </section>
        </aside>
      </div>
