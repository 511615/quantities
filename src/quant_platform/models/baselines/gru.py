...
class GRUSequenceModel(BaseModelPlugin):
    ...
    def _fit_torch(
        ...
        assert _TorchGRURegressor is not None
        ...
        network.eval()
        with torch.no_grad():
            predictions = network(sequence_tensor).detach().cpu().tolist()
        self._backend = "torch"
        self._sequence_scale = 0.0
        self._model_state = {
            "state_dict": {
            ...
            "forecast_horizon": int(
                self.spec.hyperparams.get("forecast_horizon", self.spec.target_horizon or 1)
            ),
        }
    ...
    def _predict_windows(self, windows: list[list[list[float]]]) -> list[float]:
        if self._backend == "torch" and self._model_state is not None and torch is not None:
            return self._predict_windows_torch(windows)
        return [self._predict_window_fallback(window) for window in windows]

    def _predict_windows_torch(
        self,
        windows: list[list[list[float]]],
    ) -> list[float]:  # pragma: no cover - depends on local environment
        assert torch is not None
    ...
    def _predict_window_fallback(self, window: list[list[float]]) -> float:
        if not window:
            return 0.0
        return average(window[-1]) + self._sequence_scale
    ...
    def _should_use_torch(self) -> bool:
        ...
        if self._backend_mode == "torch":
            return torch is not None and _TorchGRURegressor is not None
        return torch is not None and _TorchGRURegressor is not None
