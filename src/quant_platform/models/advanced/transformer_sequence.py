...
from quant_platform.models.support import average, load_saved_state, save_model_artifact


class TransformerSequenceModel(BaseModelPlugin):
    ...
    def fit(
        ...
        train_features = self._build_feature_matrix(train_input.blocks["market_block"])
        valid_features = (
            self._build_feature_matrix(valid_input.blocks["market_block"])
    ...
    def predict(
        ...
        _ = model_run_id
        features = self._build_feature_matrix(predict_input.blocks["market_block"])
    ...
    def _build_feature_matrix(self, windows: list[list[list[float]]]) -> list[list[float]]:
        feature_rows: list[list[float]] = []
        for window in windows:
            if not window:
                feature_rows.append([0.0, 0.0, 0.0])
                continue
            last_step = window[-1]
            first_step = window[0]
            flattened = [value for step in window for value in step]
            trend = [
                last_value - first_value
                for last_value, first_value in zip(last_step, first_step, strict=False)
            ]
            feature_rows.append(
                flattened
                + list(last_step)
                + trend
                + [average(last_step), average(flattened), float(len(window))]
            )
        return feature_rows
