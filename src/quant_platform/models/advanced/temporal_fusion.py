...
class TemporalFusionModel(BaseModelPlugin):
    advanced_kind = AdvancedModelKind.TEMPORAL_FUSION
    ...
    def __init__(self, spec) -> None:
        ...
        self._backend = "linear_temporal_fusion"
    ...
    def _build_feature_matrix(self, blocks: dict[str, object]) -> list[list[float]]:
        ...
        observed_past = blocks["observed_past"]
        known_future = blocks["known_future"]
        static_context = blocks["static_context"]
        for index in range(len(observed_past)):
            observed = flatten_numeric(observed_past[index])
            future = flatten_numeric(known_future[index])
            static = flatten_numeric(static_context[index])
            feature_rows.append(
                observed
                + future
                + static
